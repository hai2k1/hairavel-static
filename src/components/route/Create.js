...
/**
 * Filter parameters that do not need to be rendered to the component
 * @param {*} data
 * @returns
 */
...
/**
 * vue instruction execution
 * @param {*} data current node data
 * @param {*} arg execution parameter
 * @param {*} slotProps slot parameters
 */
export const vExec = function (data, arg, slotProps) {
    const { nodeName, child, vStringReplace } = data

    // Convert vModel to vBind and vOn
    ...
    if (data.vData) {
        ...
        if (!vDataTypes.includes(dataType) || !data.vData) {
            console.error('vData: invalid type', data.vData)
        } else {
            // bind data
    ...
    /**
     * Store the data created by the command in a new object
     */
    const execData = {}
    ...
    itemKeys.forEach(key => {
        ...
        if (key.startsWith('vOn')) {
            ...
            // render node transition
            const _data = key.split(':')
            // fields required by the node
 ...
 * @param {*} childNode child node and slot
 * @param {*} defaultNodes array of default slots
 * @param {*} arg variable to append to exec
 * @returns
 */
...
/**
 * Convert json to Vue template code
 */
