...
export default defineComponent({
    ...
    setup(props) {
        ...
        const delCate = (index) => {
            ...
            window.dialog.error({
                title: 'Delete prompt',
                content: `This operation is irreversible, are you sure you want to delete the "${item.name}" category? `,
                hideCancel: false,
                onOk: () => {
                    request({
        ...
        return <a-modal modalClass="file-manage page-dialog max-w-screen-md w-full" visible={this.showModal}
            ...
            <div class="arco-modal-header flex gap-2">
                <div class="flex-grow flex flex-row gap-2">
                    {!!this.filter.id && <a-upload
                        ...
                        showFileList={false}
                    >
                        {this.uploadProgress.status ? this.uploadProgress.progress + '%' : 'Upload file'}
                    </a-upload>}
                </div>
            ...
            {this.isMultiple && <div class="arco-modal-footer flex items-center gap-4 flex-row">
                    ...
                    <div class="flex gap-2 flex-nowrap flex-row flex-shrink-0">
                        {
                            this.select.map(item => <div class="relative flex-shrink-0" key={item.file_id} onClick={() => this.selectItem(item)}>
                                ...
                                <div
                                    class="absolute inset-0 bg-black bg-opacity-60 text-xs opacity-0 flex items-center justify-center text-white hover:opacity-100 select-none">delete
                                </div>
                            </div>)
                        }
                    </div>
                ...
                <div class="flex-none justify-end flex gap-2">
                    <a-button onClick={this.closeModal}>Close</a-button>
                    <a-button type="primary" onClick={this.submit}>OK</a-button>
                </div>
            </div>}
        </a-modal>
    }
