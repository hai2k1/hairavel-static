...
export default defineComponent({
    ...
    render() {
        return <a-modal modalClass={this.width} visible={this.show} title={this.title || 'select data'}
                        onCancel={this.onCancel} onClose={this.onCancel} onOk={this.onSave}
                        okButtonProps={{disabled: !this.checked.length}}>
            <div>
                <div class="flex gap-4 mb-3">
                    ...
                    {this.search && <div class="flex-none flex gap-4">
                        <div>
                            <a-input placeholder="Enter search content" vModel={[this.filter.query, 'modelValue']}/>
                        </div>
                    </div>}
                </div>
